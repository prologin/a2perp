...
class SessionDetail(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        DETAILS_ALLOWED_STATUSES = (
            models.SessionStatuses.OPEN,
            models.SessionStatuses.SUBMISSIONS_CLOSED,
        )
        return models.Session.get_user_sessions(self.request.user).filter(
            status__in=DETAILS_ALLOWED_STATUSES
        )
    ...
    def get_context_data(self, object, **kwargs):
        ...
        if object.form:
            context["form_user_answers"] = object.form.get_user_answers(
                self.request.user
            )
...
class DownloadSessionSubjectView(LoginRequiredMixin, View):
    ...
    def get(self, request, pk, **kwargs):
        session = None
        try:
            session = models.Session.get_user_sessions(request.user).get(
                pk=pk, status=models.SessionStatuses.OPEN
            )
        except ObjectDoesNotExist:
            raise Http404()
