from django.views.generic import UpdateView, ListView, FormView, DetailView, RedirectView
...
class InterviewerDispoSelect(mixins.SessionObjectMixin, mixins.EnsureInterviewerMixin, FormView):
    ...
    def get_initial(self):
        ...
        user_slots = models.SlotInstance.objects.filter(slot__session=self.session, interviewer=self.interviewer)
        initial['slot_choices'] = tuple(sloti.slot_id for sloti in user_slots)
        return initial

    def get_form_kwargs(self):
        form_kwargs = super().get_form_kwargs()
        form_kwargs['slot_choices'] = tuple((slot.id, slot.local_display) for slot in models.Slot.objects.filter(session=self.session).order_by('date_start'))
        return form_kwargs
    ...
    def form_valid(self, form=None):
        if form.has_changed():
            ...
            deleted, added = tuple(old_choices - new_choices), tuple(new_choices - old_choices)
            with transaction.atomic():
                if self.session.in_phase_1:
                    models.SlotInstance.objects.filter(interviewer=self.interviewer, slot_id__in=deleted).delete()
                for slot_id in added:
                    models.SlotInstance.objects.create(interviewer=self.interviewer, slot_id=slot_id)
...
class ContestantSlotSelect(mixins.SessionObjectMixin, LoginRequiredMixin, FormView):
    ...
    def session_passes_test(self, session):
        if self.request.user.is_staff:
            raise SuspiciousOperation('Staff is trying to sign up as contestant in phase2')
        if not session.status == models.SessionStatuses.CONTESTANT_CHOICE:
            return False
        if models.SlotInstance.objects.filter(contestant=self.request.user, slot__session=session).exists():
            return False
    ...
    def get_form_kwargs(self, *args, **kwargs):
        ...
        form_kwargs['slot_choices'] = tuple(
                (slot.id, f'{slot.local_display} ({slot.instances.filter(contestant__isnull=True).count()} en stock)')
    ...
    def form_valid(self, form=None):
        with transaction.atomic():
            try:
                sloti = (models.SlotInstance.objects
                    .select_for_update()
                    .filter(slot=form.cleaned_data['slot_choice'], contestant__isnull=True)
                    .first()
                )
                ...
                return HttpResponseGone(content_type='text/plain', content='The requested slot is out of order')
...
class InterviewRecapView(mixins.SessionObjectMixin, LoginRequiredMixin, DetailView):
    ...
    def render_detailed_404(self):
        content = template_loader.render_to_string('interviews/no-interview.html', {}, self.request)
    ...
    def get(self, request, *args, **kwargs):
        try:
            self.sloti = models.SlotInstance.objects.get(slot__session=self.session, contestant=request.user)
            return super().get(request, *args, **kwargs)
        except MultipleObjectsReturned:
            raise Exception(f'{request.user.first_name} {request.user.last_name} has multiple slot instance assignations.')
        except ObjectDoesNotExist:
            if self.session.in_phase_2:
                raise Http404
            return self.render_detailed_404()

class ContestantInterviewPortal(mixins.SessionObjectMixin, LoginRequiredMixin, RedirectView):
    def get_redirect_url(self, *args, **kwargs):
        if models.SlotInstance.objects.filter(slot__session=self.session, contestant=self.request.user).exists() or self.session.in_phase_3:
...
class InterviewerSlotsList(mixins.SessionObjectMixin, mixins.EnsureInterviewerMixin, ListView):
    ...
    def get_queryset(self):
        return models.SlotInstance.objects.filter(slot__session=self.session, contestant__isnull=False, interviewer=self.interviewer).order_by('slot__date_start')

class ContestantGrading(mixins.SessionObjectMixin, mixins.EnsureInterviewerMixin, UpdateView):
    ...
    def get_object(self, *args, **kwargs):
        ...
        if not (contestant.id,) in self.interviewer.grading_targets_queryset(self.session).values_list('contestant'):
