...
class Session(models.Model):
    ...
    def get_my_sessions(cls, user):
        if user.is_staff:
            return cls.objects.filter(status__in=(SessionStatuses.SLOT_INSTANCIATION, SessionStatuses.CONTESTANT_CHOICE, SessionStatuses.LOCKED))
        return cls.objects.filter(status__in=(SessionStatuses.CONTESTANT_CHOICE, SessionStatuses.LOCKED))
...
class Interviewer(models.Model):
    user = models.OneToOneField(to=get_user_model(), primary_key=True, limit_choices_to={'is_staff': True}, on_delete=models.CASCADE)
...
class SlotInstance(models.Model):
    slot = models.ForeignKey(to='interviews.Slot', related_name='instances', on_delete=models.CASCADE)
    interviewer = models.ForeignKey(to='interviews.Interviewer', on_delete=models.CASCADE)
    contestant = models.ForeignKey(to=get_user_model(), limit_choices_to={'is_staff': False}, on_delete=models.SET_NULL, null=True, blank=True)

    @property
    def can_see_meet_link(self):
        return (
            (self.slot.date_start - timedelta(minutes=settings.APP_ITW_MEET_UNLOCK_BEFORE)) <= timezone.now() <= self.slot.date_end
        )
...
class InterviewScore(models.Model):
    session = models.ForeignKey(to='interviews.Session', on_delete=models.CASCADE)
    contestant = models.ForeignKey(to=get_user_model(), limit_choices_to={'is_staff': False}, on_delete=models.CASCADE)
    grade = models.IntegerField(verbose_name='Note', help_text='Entier compris entre 0 et 5', validators=[validators.MinValueValidator(0), validators.MaxValueValidator(5)])
    comments = models.TextField(verbose_name='Commentaires sur l\'entretien', max_length=400)
